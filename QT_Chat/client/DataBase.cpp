...
bool DataBase::connect(QString host, unsigned int port, QString dbname, QString user, QString password)
{
    ...
    _mysql.setHostName(host);//host
    _mysql.setPort(port);//port
    _mysql.setDatabaseName(QString("DRIVER={MySQL ODBC 8.1 ANSI Driver};DBQ='%1'").
                           arg(dbname));//dbname
    _mysql.setUserName(user);//user
    _mysql.setPassword(password);//password
    if (!_mysql.open()){
        _connection = false;
        ...
        _connection = true;
        qDebug("success");
        executeQueryWithoutResult("use chatbd;");
    }
    return _connection;
}
